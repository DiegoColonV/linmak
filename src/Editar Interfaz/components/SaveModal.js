...
const SaveModal = ({ open, onClose, onSave, onShowAlert }) => {
	...
	useEffect(() => {
		if (userData.autenticado) consultarCarpetas();
	}, [open]);
	...
	return (
		<Modal show={open} onHide={onClose} centered size='lg'>
			{userData.autenticado ? (
				<div className='modal-save'>
					...
					<div className='row'>
						...
						<div className='col-6 px-4' style={{ color: '#8080ff' }}>
							...
							<ul>{selected ? selected.trabajos.map((item, i) => <li key={i}>{item.titulo_trabajo}</li>) : null}</ul>
						</div>
						<div className='row d-flex justify-content-center mt-5'>
							<div className='col-7'>
								<input disabled={!selected} type='text' value={txtPage} onChange={onChangePage} className='form-control w-100' placeholder='Nombre de la interfaz'></input>
							</div>
							<div className='col-3'>
								<Button onClick={onClickSave} variant='outline-success'>
									Guardar
								</Button>
							</div>
						</div>
				...
				<h1>Tienes que iniciar sesión para guardar</h1>
			)}
		</Modal>
	);
};
